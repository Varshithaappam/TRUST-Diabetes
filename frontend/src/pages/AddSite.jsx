...
/**
 * Add Site Form Component (Inline version)
 * Fields: site name, site address, hospital (dropdown), contact phone number, contact person name, contact person email, is_active
 */
const AddSiteForm = ({ onSuccess, onCancel }) => {
    const { token } = useAuth();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(
                'http://localhost:5000/api/users/add-site',
                payload,
                {
                    headers: {
            ...
            setError(err.response?.data?.error || err.response?.data?.message || 'Failed to add site');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-8 rounded-3xl border border-gray-100">
                    <h3 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
                        <Building2 className="w-6 h-6 text-indigo-600" /> Site Details
                    </h3>
